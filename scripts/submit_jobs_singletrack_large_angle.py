...
def main():
    ...
    EventCount=160000
    TrackerRuns=1
    Scripts= ['muon_large_angle.mac','muon_cms.mac']
    Names = ["muon"]
    CORES = 1
    ...
    sim_script = Scripts[0]
    data_dir_sub = f"{DataDir}/{Names[0]}_{energy}_GeV_large_angle/"
...
"""
    ...
    sim_script = Scripts[1]
    data_dir_sub = f"{DataDir}/{Names[0]}_{energy}_GeV_CMS/"
    job_script=f"""mkdir -p {data_dir_sub} 
{simulation} -j1 -q  -o {data_dir_sub}  -s {sim_script}  
for f in {data_dir_sub}/*/*/run*.root; do 
    {tracker} $f `dirname $f` 
...
"""
    # This is the core function to submit job script
    script_prefix=sim_script.split("_")[0]
    submit_script(job_script, f"{script_prefix}_{energy}_1", blockidx=0, hours=hours_mod, cores=CORES, log_dir=Log_Dir, job_name="reco", system=system, slurm_account=slurm_account, slurm_partition='', debug=DEBUG, verbose=verbose)    
